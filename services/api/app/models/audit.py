...
class AuditEvent(UUIDPrimaryKeyMixin, Base):
    ...
    # ``Identity(always=False)`` maps to ``GENERATED BY DEFAULT AS IDENTITY``
    # in Postgres. Migration 0006 adds the matching DDL; the older
    # ``autoincrement=True`` hint silently did nothing on non-PK columns.
